'''
A module for reading and plotting RAM-SCB output.  
'''
...
class ramsat(object):
    ...
    def plot_b(self, timelim=None, style='b-'):
        ...
        bx.grid()
        bx.plot(self.time, self.data['B_xyz'][:,0], style)
    ...
    def plot_flux(self, timelim=None):
        ...
        fx1.set_yscale('log')
        flxH = fx1.pcolor(time, egrid, self.omniH.transpose(), 
        ...
        fx2.set_yscale('log')
        flxO = fx2.pcolor(time, egrid, self.omniO.transpose(), 
                   norm=LogNorm(), vmin=1, vmax=maxFlux)
        # Nudge position
