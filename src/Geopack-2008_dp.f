...
C          #               (with a minor correction made on 04/21/08)               #
c          #                                                                        #
c          ##########################################################################
c
C
...
C=====================================================================================
C
      SUBROUTINE RHAND_F (X,Y,Z,BX,BY,BZ,IOPT,PARMOD,EXNAME,INNAME)
C
C  CALCULATES THE COMPONENTS OF THE RIGHT HAND SIDE VECTOR IN THE
C  GEOMAGNETIC FIELD
C    LINE EQUATION  (a subsidiary subroutine for the subroutine STEP_08)
C
C     LAST MODIFICATION:    MARCH 21, 2008 (DOUBLE-PRECISION VERSION)
...
C
      ...
      DIMENSION PARMOD(10)
C
C     EXNAME AND INNAME ARE NAMES OF SUBROUTINES FOR THE EXTERNAL AND
C     INTERNAL
C     PARTS OF THE TOTAL FIELD, E.G., T96_01 AND IGRF_GSW_08
C
      COMMON /GEOPACK1/ A(12),DS3,BB(2),PSI,CC(18)
...
C
C     LAST MODIFICATION:    APRIL 21, 2008
C
C     AUTHOR:  N. A. TSYGANENKO
C
      IMPLICIT REAL*8 (A-H,O-Z)
...
C
     ...
     * EXNAME,INNAME,XF,YF,ZF,XX,YY,ZZ,L,LMAX,BX,BY,BZ)
C
...
C
      ...
      DIMENSION BX(LMAX),BY(LMAX),BZ(LMAX)
...
c
  1   L=L+1
      ...
      YY(L)=Y
      ZZ(L)=Z
      CALL RHAND_F(X,Y,Z,BX(L),BY(L),BZ(L),IOPT,PARMOD,EXNAME,INNAME)
      RYZ=Y**2+Z**2
      R2=X**2+RYZ
      R=DSQRT(R2)
C
