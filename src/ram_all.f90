...
!******************************************************************************
  SUBROUTINE ram_all(IS)
    ...
    real(kind=Real8_) :: XNN(NS,NR), XND(NS,NR), LNCN(NS,NR), LNCD(NS,NR), F2r(NR,NT,NE,NPA), BNESPrev(NR+1,NT), &
                         BOUNISPrev(NR+1,NT,NPA), FNISPrev(NR+1,NT,NPA), VTOL(NR+1,NT), VTN(NR+1,NT), AVS, &
                         VT_kV(NR+1,NT), tolv, RRL, PH, lambda
    character(len=200) ST3, NameFileOut
    character(len=4) ST4 
    character(len=4) ST7no 
    character(len=2) ST8
    character(len=100) inifile
    character(len=200) hIfile
    character(len=100) HEADER
    character(len=2) DayChar, charHr, charEl2 
    character(len=4) charEl, charB
...
!	Get timetags for file names.
    call gen_old_timetags(TimeRamStart, TimeRamNow, nHourTotal=nHour,nFiveTotal=nFive, nFiveDay=nFiveDay)
...
!.......Preparation...CALL THIS ONLY ONCE TO INITIALIZE
    IF (ical.EQ.1) THEN
      ...
      elseif ((boundary .eq. 'LANL') .or. (boundary .eq. 'PTM')) then
      ...
      DO I=2,NR+1
        DO J=1,NT
          DO L=1,NPA
            if (UseEfind) then
              READ(UNITTMP_,40) RRL,PH,IPA,FNHS(I,J,L),BOUNHS(I,J,L),FNIS(I,J,L),BOUNIS(I,J,L),&
                                BNES(I,J),HDNS(I,J,L),EIR(I,J),EIP(I,J)
            else
              READ(UNITTMP_,40) RRL,PH,IPA,FNHS(I,J,L),BOUNHS(I,J,L),FNIS(I,J,L),BOUNIS(I,J,L),&
                                BNES(I,J),HDNS(I,J,L)
              EIR(I,J)=0.
    ...
    IF ((MOD(T,DT_hI) < 1e-3).and.(DoWriteFlux)) THEN
      CALL netcdf_flux_dump(FLUX(S,:,:,:,:), NR, LZ, NT, PHI, NE, EKEV,NPA, MU, S, UTs)
    ENDIF
    ...
    IF (MOD(INT(T),INT(Dt_bc)).EQ.0 .and. ical.GT.1) THEN
      IF (S.EQ.4) THEN
        ...
        if (boundary .EQ. 'LANL') print*, 'RAM: calling GEOSB at time (hr) = ', T/3600.
      ENDIF
      CALL GEOSB(nFive)
      IF (DoUseWPI) THEN
        IF (S.EQ.1) THEN
...
!**************************************************************************
  SUBROUTINE WAPARA_Kp()
    ...
    if (Kp.gt.maxval(Kp_chorus)) then
      ...
      CDAAR(:,:,:,:) = (Kp-Kp_chorus(i1))*CDAAR_chorus(:,:,1:35,:,i2) + (Kp_chorus(i2)-Kp)*CDAAR_chorus(:,:,1:35,:,i1)
  ...
  SUBROUTINE WAPARA_BAS
    ...
    do nkp=1,nloop
      ...
      do i=1,NR_Dxx
        do j=1,NT_Dxx
          do k=1,NE_Dxx
            read(UNITTMP_,'(A19,F6.4,A9,F6.4,A12,F8.2)') H1,RCHOR_Dxx(i), H2, TCHOR_Dxx(j), H3, ECHOR_Dxx(k)
            do l=1,NPA_Dxx
...
!**************************************************************************
  SUBROUTINE INITIAL(LNCN,LNCD,XNN,XND)
    ...
    if (.not.IsRestart)then
      ...
      OPEN(UNIT=UNITTMP_,FILE=ST4(1:inBlank)//'Frbspmar13'//ST2//'.in',STATUS='OLD')
...
25  if (.not.IsRestart)then
      ...
      if (nT==25) then
        open(unit=UNITTMP_,file=ST4(1:inBlank)//'f2inirc'//ST2//'.dat',status='old') 
      else 
...
!**************************************************************************
  SUBROUTINE WRESULT(LNCN,LNCD,XNN,XND)
    ...
    real(kind=Real8_) :: LNCN(NS,NR),LNCD(NS,NR),XNN(NS,NR),XND(NS,NR),XNNO(NR),XNDO(NR)
        ...
        DO L=1,NPA
          DO J=1,NT-1
            ...
            if (f(i,j,k,l).lt.0) then
              iUnit7 = io_unit_new()
              open(UNIT=iUnit7,FILE=trim(ST4)//'fch'//ST0//ST1//ST2//'.dat',STATUS='UNKNOWN',ACCESS='APPEND')
              write(iUnit7,*)' f=',f(i,j,k,l),' i=',i,' j=',j,' k=',k,' l=',l,'  T=',T/3600.
              f2(S,i,j,k,l)=1E-15
              f(i,j,k,l)=1E-15
              close(iUnit7)
            endif
      ...
      DO 1 K=2,NE
        DO 1 I=2,NR
          ...
          DO J=2,NT
            J1=J+1
            IF (J.EQ.NT) J1=2
            GPA1=FNIS(I,J,L)+FNIS(I,J1,L)+(FNIS(I+1,J1,L)+FNIS(I+1,J,L)-FNIS(I-1,J,L)-FNIS(I-1,J1,L))*Z(I)/2./DR
            GPA2=Z(I)/4./DR*(FNIS(I,J,L)+FNIS(I,J1,L)-2*FNHS(I,J,L)-2*FNHS(I,J1,L)) &
                *(BNES(I+1,J1)+BNES(I+1,J)-BNES(I-1,J)-BNES(I-1,J1))/(BNES(I,J)+BNES(I,J1))
            GPA=GPA1+GPA2
            VPA(I,J,K,L)=P2(I,K)*GPA/(FNHS(I,J,L)+FNHS(I,J1,L))/(BNES(I,J)+BNES(I,J1))*DPHI/DT*Z(I)
    ...
    DO 1 J=1,NT
      ...
      DO 1 I=2,NR
        ...
        DO 1 L=2,NPA
          ...
          GPP(I,J,L)=GPP1+GPP2
          DRD2(I,J,L)=(FNIS(I,J2,L)-FNIS(I,J0,L))/2./DPHI+(FNIS(I,J,L)-2*FNHS(I,J,L)) &
                     *(BNES(I,J2)-BNES(I,J0))/4/BNES(I,J)/DPHI
          DPD2(I,J,L)=FNIS(I,J,L)+(FNIS(I+1,J,L)-FNIS(I-1,J,L))*Z(I)/2/DR &
                     +Z(I)*(FNIS(I,J,L)-2*FNHS(I,J,L))/4/DR*(BNES(I+1,J)-BNES(I-1,J))/BNES(I,J)
          DO K=2,NE
            F(K)=F2(S,I,J,K,L)
          END DO
          ...
          DO K=1,NE
            EDT1=EBND(K)*1e3*(GRBND(K)+1)/2/GRBND(K)/FNHS(I,J,L)/Z(I)/BNES(I,J)/QS
      ...
      DO 1 J=1,NT
        ...
        DO 1 I=2,NR
          ...
          DO L=2,NPA
            ...
            GMP(I,J,L)=GMP1+GMP2
            EDT(L)=EKEV(K)*1e3*(GREL(K)+1)/2/GREL(K)/BOUNHS(I,J,L)/Z(I)/BNES(I,J)/QS
            DRM2(I,J,L)=(BOUNIS(I,J1,L)-BOUNIS(I,J0,L))/2/DPHI+(BOUNIS(I,J,L)-2*BOUNHS(I,J,L)) &
                       *(BNES(I,J1)-BNES(I,J0))/4/BNES(I,J)/DPHI
            DPM2(I,J,L)=BOUNIS(I,J,L)+(BOUNIS(I+1,J,L)-BOUNIS(I-1,J,L))*Z(I)/2/DR+(BOUNIS(I,J,L)-2*BOUNHS(I,J,L)) &
                       *Z(I)/4/DR*(BNES(I+1,J)-BNES(I-1,J))/BNES(I,J)
...
!**************************************************************************
  SUBROUTINE GEOSB(nFive)
    ...
    use ModRamMain
    use ModRamCouple, ONLY: FluxBats_IIS, generate_flux, TypeMHD, MhdDensPres_VII,FluxBats_anis
    use ModIoUnit,    ONLY: UNITTMP_
          ...
          do iJ=1,nT
            do L=2,UPA(NR)-1
              if (.not.DoAnisoPressureGMCoupling) then
                ...
                FGEOS(s,iJ,iK,L)=FluxBats_anis(iK,L,iJ,s)*FFACTOR(NR,iK,L)
              endif
            end do
          end do
        end do
    ...
    if (UseNewFmt) then
      NameFluxFile=trim(PathRamOut)//RamFileName('Dsbnd/ds'//St2,'dat',TimeRamNow)
    else
      call gen_old_timetags(TimeRamStart, TimeRamNow, nFiveDay=nFiveDay)
      write(NameFluxFile,'(a,i4.4,a)')trim(PathRamOut)//'Dsbnd/ds_',nFiveDay,ST2//'.dat'
    end if
...
!*************************************************************************
  SUBROUTINE ANISCH
    ...
    IF (DoUseWPI.and.MOD(INT(T),3600).EQ.0.) THEN
      OPEN(UNIT=UNITTMP_,FILE=trim(PathRamOut)//ST0//ST1//ST2//'.wans',STATUS='UNKNOWN')
    ...
    DO 15 I=2,NR
      ...
      DO 15 J=1,NT
        ...
        IF (DoUseWPI.and.MOD(INT(T),3600).EQ.0.) WRITE(UNITTMP_,551) LZ(I),PHI(J),ANIST,EDENT,RNHTT,PPERT(S,I,J),PPART(S,I,J)
15  CONTINUE
    CLOSE(UNITTMP_)
...
17    CONTINUE
      ...
      OPEN(UNIT=UNITTMP_,FILE=trim(PathRamOut)//ST0//ST1//ST2//'.wdc',STATUS='UNKNOWN')
          ...
          IF (XNE(I,J).LE.50.) THEN ! outside pp
            ...
            DO K=2,NE
              ...
              DO L=1,NPA
                ...
                if (l.eq.2.or.l.eq.6.or.l.eq.10.or.l.eq.15.or.l.eq.20.or.l.eq.25.or.l.eq.30.or.l.eq.40.or.l.eq.50) then
...
41    CONTINUE
      ...
      Bw=30.
      IF(KP.GE.4.0) Bw=100.                                 ! pT
      OPEN(UNIT=UNITTMP_,FILE=trim(PathRamOut)//ST0//ST1//ST2//'.wdh',STATUS='UNKNOWN')
      ...
      DO 401 I=2,NR
        DO 401 J=1,NT
          IF (XNE(I,J).GT.50.) THEN ! inside pp
            ...
            write(UNITTMP_,550)
            write(UNITTMP_,553) LZ(I),MLT(J),fnorm,XNE(I,J),' omega=',omega,' Bhiss=',Bw
            DO 402 L=1,NPA
              MUBOUN=MU(L)+0.5*WMU(L)
              DO IZ=1,NCF
                DO KN=1,ENG 
              ...
              DO 402 K=2,NE
                ...
                DWAVE(L)=10.**Y*fnorm/GREL(K)**2*(1.-MUBOUN*MUBOUN)/MUBOUN ! denorm pa [1/s]
...
56 FORMAT(2HT=,F8.3,2X,3HKp=,F3.1,/,2X,'L   PHI     ANIS   EDEN[keV/cm3]  RNHT[1/cm3]  PPER[keV/cm3]','  PPAR[keV/cm3]')
550 format(1X,36hbounce-aver diff coeff & time scales,/,1X,4Hekev,4X,4Heqpa,4X,9Hdaa [1/s],4X,8Htdaa [s])
551 FORMAT(F5.2,F10.6,10(1PE12.3))
552 format(2F9.3,6(1PE12.3))
553 format(3H L=,F5.2,5H MLT=,F5.2,7H fnorm=,E10.3,4HNe=,F9.2,A7,E10.3,A7,E10.3)
