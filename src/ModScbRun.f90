...
!        IF (isotropy == 1) CALL entropy(entropyFixed, fluxVolume, iCountEntropy)
     ...
     ! PRINT*, 'CE: blendPsi = ', blendPsi
        ...
        END IF
        WRITE(*,*) ' itout ',' blendAlpha ',' blendPsi ',' itAlpha ',' diffAlpha ',' errorAlpha ',' itPsi ',' diffPsi ',' errorPsi '
...
!  CALL plot_physical   ! Writes physical quantities to disk

!  IF (isotropy == 0) CALL computehI(fluxVolume) ! Coupling with RAM
...
SUBROUTINE findR
  ...
  USE ModScbMain,      ONLY: DP, iAzimOffset
...
SUBROUTINE energy
  ...
  USE ModScbMain,  ONLY: DP, isotropy
