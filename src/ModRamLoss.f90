...
! currently contains charge exchange and atmospheric loss
  ...
  real(kind=Real8_), ALLOCATABLE :: CHARGE(:,:,:,:), ATLOS(:,:)
  !$OMP THREADPRIVATE(CHARGE, ATLOS)
...
!==============================================================================
  SUBROUTINE LOSSEND
    ...
    DEALLOCATE(CHARGE, ATLOS)
 
  END SUBROUTINE LOSSEND
...
!**************************************************************************
  SUBROUTINE CEPARA(S)
    ...
    !$OMP THREADPRIVATE(X, Y, ALPHA, TAUB)

    ALLOCATE(CHARGE(nR,nT,nE,nPa), ATLOS(nR,nE))
  ...
  SUBROUTINE CHAREXCHANGE(S)
    ...
    DO K=2,NE
       DO J=1,NT
          DO I=2,NR
             DO L=2,NPA
                F2(S,I,J,K,L)=F2(S,I,J,K,L)*CHARGE(I,J,K,L)
             ENDDO
          ENDDO
       ENDDO
    ENDDO
