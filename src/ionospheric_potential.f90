SUBROUTINE ionospheric_potential
  ...
  use ModRamMain,   ONLY: IsComponent, PathSwmfOut, TimeRamNow
  ...
  ! Periodicity
  colatGrid(:,nzetp) = colatGrid(:,2)
  lonGrid(:,nzetp) = lonGrid(:,2) + 2.*pi_d 
  latGrid(:,nzetp) = latGrid(:,2)
  colatGrid(:,1) = colatGrid(:,nzeta)
  lonGrid(:,1) = lonGrid(:,nzeta) - 2.*pi_d 
  latGrid(:,1) = latGrid(:,nzeta)

  SELECT CASE (iElectric)

  CASE(1) ! interpolate SWMF iono potentials on 3Dcode ionospheric grid
     if(IsComponent) then
     ...
     CALL Spline_2D_periodic(colat, lon, PhiIonoRaw, colatGrid(1:npsi,2:nzetp), lonGrid(1:npsi,2:nzetp), &
          PhiIono(1:npsi,2:nzetp), iDomain)
  ...
  CASE(2) ! Potential by calling Weimer function 
     UseAL = .TRUE.		
...
101  FORMAT(2I4, 2I3, 3F8.2, F8.1, F7.2, F6.2, 2I6)
  ...
  CASE(3) ! Potential by calling Weimer 2005 function 'W5SC'
     open(Unittmp_, file=NameOmniFile, status = 'unknown',action='read')
     i = 0
     ierr = 0
     do while (ierr == 0)
  ...
  dPhiIonodRho(:,nzetp) = dPhiIonodRho(:,2)
  dPhiIonodRho(:,1) = dPhiIonodRho(:,nzeta)
  dPhiIonodZeta(:,nzetp) = dPhiIonodZeta(:,2)
  dPhiIonodZeta(:,1) = dPhiIonodZeta(:,nzeta)

  DO j = 1, npsi
     dPhiIonodAlpha(j,1:nzetp) = dPhiIonodRho(j,1:nzetp) / f(j)
  END DO
  DO k = 1, nzetp
