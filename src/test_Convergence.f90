SUBROUTINE test_Convergence
  ...
  REAL(DP) :: normDiff, normDiffRel, normJxB, normGradP, volume
  ...
  jCrossBMinusGradPMod = SQRT(jCrossBMinusGradPSq)
  gradP = SQRT(gradPSq)

  ! Force balance quantities for isotropic case
  OPEN(UNITTMP_, file = 'Force_balance_equatorial', status = 'replace')  ! for Matlab plotting
  ...
  OPEN(UNITTMP_, file ='Force_balance_midnight', status = 'replace') 
  WRITE(UNITTMP_, '(3I7)') nthe, npsi, nzeta
  DO j = 1, npsi
     DO i = 1, nthe
  ...
  ! Only compute norms for half-space, due to the N-S symmetry
  DO i = 2, nThetaEquator  
     DO j = 2, npsi-1
        DO k = 2, nzeta
           non_CurrentSheet:    IF (iPressureChoice /= 2) THEN
              IF (x(i,j,k) < -4._dp) THEN   
              ...
              END IF
           ELSE   ! Current sheet case, 
              IF (ABS(x(i,j,k)) > 4._dp) THEN  ! Only for |X| > 4 R_E, closer distances not considered
                 normDiff = normDiff + jacobian(i,j,k) * dr * dpPrime * dt * &
                      jCrossBMinusGradPMod(i,j,k)
                 normDiffRel = normDiffRel + jacobian(i,j,k) * dr * dpPrime * dt * &
                      jCrossBMinusGradPMod(i,j,k) / jCrossB(i,j,k) 
                 ...
                 volume = volume + jacobian(i,j,k) * dr * dpPrime * dt
              END IF
           END IF non_CurrentSheet
        END DO
     END DO
  END DO

  !  Norms of |jxB-grad P|,      |jxB|,      |gradP| 
  WRITE(*, '(3 (3X, E14.5))') normDiff/volume, normJxB/volume, normGradP/volume; CALL FLUSH(6)
  WRITE(iUnitLog, '(3 (3X, E14.5))') normDiff/volume, normJxB/volume, normGradP/volume; CALL FLUSH(iUnitLog)
